...
struct test_nightbane : public BossAI
{
    test_nightbane(Creature* creature) : BossAI(creature, DATA_NIGHTBANE)
    {
        _intro = true;
        _skeletonCount = 5;
        _movePhase = 0;
    ...
    void TakeOff()
    {
        ...
        me->GetMotionMaster()->MovePoint(0, IntroWay2[2][0], IntroWay2[2][1], IntroWay2[2][2]);
        ...
        scheduler.Schedule(45s, 60s, [this](TaskContext)
        {
            ...
            me->GetMotionMaster()->Clear(false);
            me->GetMotionMaster()->MovePoint(3, IntroWay2[3][0], IntroWay2[3][1], IntroWay2[3][2]);
        ...
        if (_intro)
        {
            if (_movePhase)
            {
                if (_movePhase >= 7)
                {
                    ...
                    me->GetMotionMaster()->MovePoint(8, IntroWay2[7][0], IntroWay2[7][1], IntroWay2[7][2]);
                }
                else
                {
                    me->GetMotionMaster()->MovePoint(_movePhase, IntroWay2[_movePhase][0], IntroWay2[_movePhase][1], IntroWay2[_movePhase][2]);
                }
                _movePhase = 0;
            }
            return;
...
public:
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        ...
        {
            if (instance->GetData(DATA_NIGHTBANE) != DONE && !go->FindNearestCreature(385000, 40.0f))
            {
                if (Creature* nightbane = instance->GetCreature(DATA_NIGHTBANE))
                {
                    nightbane->GetMotionMaster()->MovePoint(0, IntroWay2[0][0], IntroWay2[0][1], IntroWay2[0][2]);
...
{
    RegisterKarazhanCreatureAI(test_nightbane);
